...
protocol SuggestionProvider: Sendable {
    ...
    /// Record a navigation so it can rank in future suggestion queries.
    /// Default impl is a no-op so mocks don't have to care.
    func recordVisit(title: String, url: String) async

    /// Flag a URL as bookmarked. Bookmarks get a ranking boost and surface
