...
**Rule:** Patches against the **WebKit checkout** live under **`webkit/patches/`**. Optional **extra** layers (examples: Fluent/Mica MiniBrowser demo, future passkeys stubs) live in **`changes/<lane>/patches/`** and are off by default—see `config/changes.json` and `changes/README.md`. WebGPU/Dawn work was merged into `webkit/patches/windows/`.
